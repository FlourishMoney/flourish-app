...
**Number-invention is now defended at three layers for facilitator, chat AND checkin:** the fixed judge (grades against the agenda/context), the hardened prompts, and the deterministic guards — the hard guarantee, independent of model non-determinism. The chat/checkin guard (`netlify/functions/_lib/snapshotGuard.js`, `8607436`) **reuses the facilitator validator** over the snapshot the client sends as `context` plus the numbers the user typed; it rejects any dollar figure, rate, date, count, percentage or derived statistic (including arithmetic on snapshot numbers — a computed surplus/total/average) that isn't a supplied fact, and program limits/amounts not in the snapshot (program names alone are fine), with the same **validate → one retry naming the figure → safe qualitative fallback** handling and the client reply footer preserved. **Simulator, document-import and facilitator behaviour were not altered.** Unit-tested in `tests/snapshotGuard.test.cjs` (17 assertions, incl. exact-figure-allowed, computed-surplus-rejected, misquote-rejected, invented-percentage/total-rejected, user-typed-figure-allowed, failed-retry-fallback). After the extension, two more full live QA runs: **28/28 · 0 failed · 0 errored** each. The offline gate is now **35 suites**. Synthetic data only; no real financial snapshot was sent.
...
1. **DONE:** live coach QA is **28/28, green across 5 runs** (3 after the fix pass + 2 after the guard extension); all number-invention failures fixed. The deterministic numeric guard now covers **facilitator, chat and checkin**. Prompt caching **engages** (~1,198-token stable prefix, 1,173 tokens read/call). The remaining pre-deploy item is **Netlify preview isolation** — move the four Plaid/Supabase secrets + `ANTHROPIC_API_KEY` to Production-only (see the separate audit already delivered).
