...
**Branch:** `strategy-implementation` @ `c33c41e` (not merged, not deployed)
...
- **§4 Landing — DEFERRED BY DECISION (architecture).** The landing is now the redesigned **waitlist-first** page (DECISIONS 7). None of §4's anchor strings (old hero, pain points, how-it-works steps, pricing grid) exist; the mapping pass confirmed every row is not-applicable or already-satisfied (the read-only trust chip is present; no PIPEDA claim). The approved marketing copy for those removed sections applies once billing/full-marketing ship — tracked, not lost.
