  ...
  if (hasStage("senior", "retired")) {
    if (country === "CA") {
      tips.unshift(
        ...
        {title:"OAS & GIS — Are You Getting Everything?",body:`Old Age Security (~$${TAX_DATA.CA.OAS_MAX_MONTHLY.value.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}/mo at 65, approximate — Apr–Jun 2026 quarter) is automatic, but the Guaranteed Income Supplement (GIS) is not — you must apply. Low-income seniors leave GIS unclaimed every year. If your income is under ~$21,624, apply immediately.`,savings:"Up to $1,065/mo (GIS)",flag:"🇨🇦",priority:"high",action:"Apply at Service Canada"},
      {title:"CPP Maximum — Know What You're Entitled To",body:`The maximum CPP retirement pension is $${TAX_DATA.CA.CPP_MAX_MONTHLY.value.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}/mo (2026) at age 65. Your actual amount depends on contributions history. You can check your CPP Statement of Contributions at My Service Canada Account.`,savings:`Up to $${TAX_DATA.CA.CPP_MAX_MONTHLY.value.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}/mo`,flag:"🇨🇦",priority:"medium",action:"Check My Service Canada"},
...
// ── FLOURISH LOGO MARK ────────────────────────────────────────────────────────
// Flourish symbol — OUR real diamond-leaf mark, cropped from the source art with the wordmark removed
// (transparent, works on light + dark). Symbol-only: used in nav, headers, favicon and all small
// placements. The full symbol+wordmark lockup lives in <FlourishLockup> (big brand moments only).
function FlourishMark({ size = 24, style = {} }) {
  return (
    <img src="/flourish-symbol.png" width={size} height={size} alt="" aria-hidden="true"
      style={{ flexShrink: 0, display: "block", objectFit: "contain", ...style }} />
  );
}
// Kids has its own distinct symbol (lime-green sprout), likewise cropped text-free from the kids art.
function FlourishMarkKids({ size = 24, style = {} }) {
  return (
    <img src="/flourish-kids-symbol.png" width={size} height={size} alt="" aria-hidden="true"
      style={{ flexShrink: 0, display: "block", objectFit: "contain", ...style }} />
  );
}
// Full lockup (symbol above wordmark) — reserved for big centered brand moments (welcome splash,
// auth / reset / MFA screens). Everywhere else uses the symbol-only <FlourishMark>. `color` is the
// wordmark color; the default suits the dark brand screens — pass C.cream on themed surfaces.
function FlourishLockup({ size = 72, color = "#EDE9E2", style = {} }) {
  return (
      ...
      {!showAuth ? (
        ...
        <div className="fll-root" style={{ display: "flex", flexDirection: "column", minHeight: "100dvh", background: "#FDF6EC" }}>
          ...
          <div className="fll-wrap fll-hero">
            <span className="fll-badge" aria-label="Coming soon to iOS, Android, and Windows">
              <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }} aria-hidden="true">
                ...
                {/* Microsoft */}
                <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><path d="M2 2h9.2v9.2H2zM12.8 2H22v9.2h-9.2zM2 12.8h9.2V22H2zM12.8 12.8H22V22h-9.2z"/></svg>
              </span>
              Coming soon
            </span>
